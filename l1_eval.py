...
def surv_diff(model, estimate, x, steps):
    ...
    integ = torch.sum(torch.diff(torch.cat([torch.zeros((surv1.shape[0], 1), device=x.device), time_steps], dim=1)) * (
        torch.abs(surv1 - surv2)), dim=1)
...
def BS_censored(t, event_t, x, model, e, km_h, km_p):
    ...
    return torch.mean(
        (tmp * (t_ind1 / (G_event + 1e-9 * (G_event == 0)))) + (tmp * (t_ind2 / (G_t + 1e-9 * (G_t == 0)))))
...
def evaluate_IBS(dep_model, indep_model, dgp, test_dict, km_h, km_p, E_reverse):
    ...
    dep_obs = IBS(test_dict['T'], test_dict['X'], dep_model, torch.max(test_dict['T']), E, km_h,
                  km_p).cpu().numpy().item()
    indep_obs = IBS(test_dict['T'], test_dict['X'], indep_model, torch.max(test_dict['T']), E, km_h,
                    km_p).cpu().numpy().item()
    return [[dgp_obs, dep_obs, indep_obs], [dgp_tot, dep_tot, indep_tot]]
