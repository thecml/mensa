...
results_path=$base_path/../results/synthetic_me.csv
    ...
    for linearity in "${linearities[@]}"; do
        for k_tau in "${k_taus[@]}"; do
            for seed in "${seeds[@]}"; do
                ...
                python3 $base_path/../src/experiments/run_dgp_multi_event.py --seed "$seed" --k_tau "$k_tau" --copula_name "$copula_name" --linear "$linearity"
            done
        done
    done
done