...
from utility.tuning import get_coxboost_sweep_cfg, get_deephit_sweep_cfg, get_deepsurv_sweep_cfg
...
def train_model():
    ...
    wandb.init(config=config_defaults, group=dataset_name, tags=["deephit"])
    config = wandb.config
    ...
    # Make predictions
    model_preds = model.predict_surv(valid_dict['X']).cpu().numpy()
