...
from utility.tuning import get_nfg_sweep_cfg
from utility.config import load_config
from utility.survival import convert_to_structured, make_time_bins, preprocess_data
from data_loader import get_data_loader
from nfg.nfg_api import NeuralFineGrayTorch 
from sota_models import make_nfg_model
import warnings
import random
import pandas as pd
...
def main():
    ...
    sweep_config = get_nfg_sweep_cfg()
    sweep_id = wandb.sweep(sweep_config, project=f'{PROJECT_NAME}')
    wandb.agent(sweep_id, train_model, count=N_RUNS)

def train_model():
    # Initialize a new wandb run
    config_defaults = cfg.NFG_PARAMS
    wandb.init(config=config_defaults, group=dataset_name, tags=["nfg"])
    config = wandb.config
    ...
    train_dict['T'] = get_first_event(torch.tensor(train_dict['T'], device=device, dtype=torch.float64))
    valid_dict['X'] = torch.tensor(X_valid, device=device, dtype=dtype)
    valid_dict['E'] = get_first_event(torch.tensor(valid_dict['E'], device=device, dtype=torch.int32))
    valid_dict['T'] = get_first_event(torch.tensor(valid_dict['T'], device=device, dtype=torch.float64))
    test_dict['X'] = torch.tensor(X_test, device=device, dtype=dtype)
    test_dict['E'] = get_first_event(torch.tensor(test_dict['E'], device=device, dtype=torch.int32))
    test_dict['T'] = get_first_event(torch.tensor(test_dict['T'], device=device, dtype=torch.float64))
    ...
    # Make predictions
    model_preds = model.predict_survival(valid_dict['X'].cpu().numpy(),
