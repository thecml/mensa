...
import copy
from dgp import Weibull_linear, Weibull_nonlinear, Weibull_log_linear, Exp_linear, DGP_LogNormal_linear, LogNormalCox_linear, DGP_LogNormal_nonlinear
...
# Setup device
device = 'cpu' #torch.device('cuda:5' if torch.cuda.is_available() else 'cpu')
...
if __name__ == "__main__":
    ...
    model1 = DGP_LogNormal_nonlinear(n_features, 4, device = device)
    model2 = DGP_LogNormal_nonlinear(n_features, 4, device = device)
    ...
    model1, model2, copula = train_mensa_model_2_events(train_dict, valid_dict, model1, model2,
                                                        copula, n_epochs=20000, lr=0.001, model_type='LogNormal_nonlinear')
